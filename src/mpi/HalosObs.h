...
public:
    ...
    [[nodiscard]] Eigen::Map<const SU3> view_link_halo_obs_const(face f, buf b, size_t site,
                                                                 int mu) const {
        if (b == send) {
    ...
    [[nodiscard]] SU3 get_link_with_halo_obs(const GaugeField& field, const GeometryCB& geo, int x,
                                             int y, int z, int t, int mu) const {
        if (x >= 0 && y >= 0 && z >= 0 && t >= 0 && x <= L - 1 && y <= L - 1 && z <= L - 1 &&
            t <= L - 1) {
